 ...
 RETURNS TRIGGER AS
    ...
    BEGIN
        IF (tg_op = 'INSERT') THEN
	        count_ratings := (SELECT count(review_id) FROM reviews WHERE book_id = new.book_id);
	        old_rating := (SELECT book_rating FROM books WHERE book_id = new.book_id);
	        new_rating := (old_rating * count_ratings + new.rating)/(count_ratings + 1);
...
CREATE TRIGGER books_rating
    ...
    FOR EACH ROW EXECUTE PROCEDURE books_calculate_rating();